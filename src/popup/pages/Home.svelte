<script>
	...
	// Typewriter
	let people = ["articles", "research papers", "blog posts", "essays","You"]
	// Logo animation
		...
		<Row>
          ...
          <g filter="url(#null)">
            ...
            {/if}
				...
				<Button icon="{ArrowRight32}" size="lg"
				 href={
				 nav.length >
		...
		<Row>
			<Column md sm="{breakpoints.sm}">
				<Tile class="my--tile">
					...
					<h4>
						Eiusmod mollit eiusmod nisi amet proident enim Lorem. Ea non eiusmod aliqua tempor consequat sit
						non elit vite and svelte.
					</h4>
				</Tile>
			</Column>
			<Column md sm="{breakpoints.sm}">
				<Tile class="my--tile">
					<h2 class="tile--title">Robust</h2>
					<h4>
						Eiusmod mollit eiusmod nisi amet proident enim Lorem. Ea non eiusmod aliqua tempor consequat sit
						non elit robust much.
					</h4>
				</Tile>
			</Column>
			<Column md sm="{breakpoints.sm}">
				<Tile class="my--tile"
					><h2 class="tile--title">Copy + Paste</h2>
					<h4>
						Eiusmod cillum consectetur in reprehenderit laborum aliqua est sunt commodo elit est veniam
						tempor veniam. Occaecat pariatur culpa velit proident commodo ex sunt nulla deserunt et copy and
						paste.
					</h4></Tile
				>
			</Column>
			<Column md sm="{breakpoints.sm}">
				<Tile class="my--tile"
					><h2 class="tile--title">Education</h2>
					<h4>
						Ut excepteur incididunt eiusmod est. Veniam sit veniam do sit reprehenderit ea nisi elit sint
						nostrud lo education.
					</h4></Tile
				>
			</Column>
		</Row>
...
<style>
	...
	:global(.my--line-wrapper) {
		height: 100px;
	}
	:global(.my--imagePromo-wrapper) {
		text-align: center;
	}
	:global(.my--tile) {
		height: 100%;
	...
	.my--imagePromo {
		...
		filter: drop-shadow(1px 1px 0 var(--cds-interactive-01)) drop-shadow(-1px -1px 0 var(--cds-interactive-01));
	}
	.my--imagePromo.browser {
